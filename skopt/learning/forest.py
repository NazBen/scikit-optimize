...
class RandomForestQuantileRegressor(sk_RandomForestRegressor):
    """
    QuantileRandomForestRegressor that supports `return_std`.
    """
    def __init__(self, quantiles, n_estimators=10, criterion='mse', max_depth=None,
                 min_samples_split=2, min_samples_leaf=1,
                 min_weight_fraction_leaf=0.0, max_features='auto',
                 max_leaf_nodes=None, bootstrap=True, oob_score=False,
                 n_jobs=1, random_state=None, verbose=0, warm_start=False,
    ...
    def predict(self, X, return_std=False):
        ...
        * `std` [array-like, shape=(n_samples,)]:
            ...
            is set to "mse", then `std[i] ~= std(y | X[i])`.
        """
        mean = super(RandomForestQuantileRegressor, self).predict_quantile(X, self.quantiles)
